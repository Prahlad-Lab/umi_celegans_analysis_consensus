...
gtf <- rtracklayer::import(con = "Y:/Johnny/Roswell_Projects/Sehee_mRNA_piRNA/Github.Sehee.paper/Data/input/Caenorhabditis_elegans.WBcel235.111.gtf.gz")
...
analyze_deg_subset <- function(sheet_name, label, variants_df, intron_df) {
  ...
  p_global <- ggplot(deg_error_rate_data, aes(x = Sample, y = Error_Rate, fill = Sample)) +
    ...
    labs(
      ...
      subtitle = paste("Wilcoxon p-value:", format.pval(stat_test$p, digits = 3)),
      x = "Sample",
  ...
  if(nrow(deg_intron_variants) > 0) {
    ...
    if(nrow(intron_only_data) > 0) {
      ...
      cat(paste0("Info: No variant data found for 'Genes with Introns' in subset: ", label, "\n"))
    }
    ...
    if(nrow(no_intron_data) > 0) {
      p_no_intron <- ggplot(no_intron_data, aes(x = Sample, y = Error_Rate, fill = Sample)) +
       ...
       cat(paste0("Info: No variant data found for 'Genes without Introns' in subset: ", label, "\n"))
    }
    
  } else {
    cat("Insufficient data for Intron analysis on this subset.\n")
  }
}
