...
#SBATCH --output=Consensus_UMI_Pipeline.out
...
BASE_OUTPUT_DIR="output_v8_consensus"
...
run_fastq_to_ubam() {
    ...
    gatk FastqToSam SORT_ORDER=unsorted F1=${DIR_IN}/${sample}_R1.subset.fastq.gz F2=${DIR_IN}/${sample}_R2.subset.fastq.gz \
...
run_extract_umis() {
    ...
    fgbio ExtractUmisFromBam --input ${DIR_IN}/${sample}.unmapped.bam --read-structure 8M6S+T --molecular-index-tags RX --output ${output_file}
}
...
# --- Step 9: Group (UMI Tools) ---
run_group_umis() {
    sample=$1; DIR_IN=$2
    output_file="${DIR_IN}/${sample}.grouped_by_UMI.bam"
    if [ -s "${output_file}" ]; then echo "[SKIP] GroupByUMIs ${sample}"; return 0; fi

    echo "[RUN] GroupByUMIs ${sample}"
    export PYTHONHASHSEED=0
    umi_tools group -I ${DIR_IN}/${sample}.MergeBamAlignment.coordinate_sorted.bam --paired --no-sort-output --output-bam \
        --stdout ${output_file} --umi-tag-delimiter "-" --log ${DIR_IN}/${sample}.group.log \
        --group-out ${DIR_IN}/${sample}.grouped.tsv --extract-umi-method tag --umi-tag RX --unmapped-reads use
}
export -f run_group_umis

# --- Step 9.5: Consensus Analysis ---
consensus_analysis() {
    ...
    # 1. Group (fgbio)
    fgbio GroupReadsByUmi --input ${DIR_UMI_MARKING}/${sample}.MergeBamAlignment.coordinate_sorted.bam \
        --strategy Adjacency --edits 1 --output ${DIR_CONSENSUS}/${sample}.fgbio_grouped.bam
    # 2. Call Consensus (min-reads 1 keeps singletons as consensus reads)
    ...
    # 4. Merge Consensus
    gatk MergeBamAlignment --REFERENCE_SEQUENCE ${REF_GENOME} --UNMAPPED_BAM ${DIR_CONSENSUS}/${sample}.consensus.unmapped.bam \
        --ALIGNED_BAM ${DIR_CONSENSUS}/${sample}.consensus.Aligned.out.bam --OUTPUT ${final_output} \
        --INCLUDE_SECONDARY_ALIGNMENTS false --VALIDATION_STRINGENCY SILENT
        
    # 5. Metrics (Optional - kept for error rate check)
...
echo "--- Step 1: FastQC ---"
fastqc -t ${SLURM_CPUS_PER_TASK:-16} -q -o ${DIR_FASTQC} $(ls ${DIR_SEQS}/*fastq.gz)
...
parallel -j 6 run_star_align {} "${DIR_EXTRACT_UMIS}" "${DIR_STAR_ALIGN}" "${STAR_INDEX}" ::: "${sample[@]}"
...
echo "--- Step 9: GroupByUMIs (for stats) ---"
parallel -j 6 run_group_umis {} "${DIR_UMI_MARKING}" ::: "${sample[@]}"


# --- Consensus Generation (Step 9.5) ---
echo "--- Step 9.5: Consensus Analysis ---"
parallel -j 6 consensus_analysis {} "${DIR_UMI_MARKING}" "${DIR_CONSENSUS}" "${STAR_INDEX}" "${REF_GENOME}" ::: "${sample[@]}"
